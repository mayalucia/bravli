"""
PSP validation tool will sample connected pairs in a pathway specified
as a named pre-synaptic target cell group, and a named post-synaptic target
cell group.
The pre/post target cell groups can be specified in a `targets.yaml`.

This app can be used to generate such targets.
"""
...
def main(args):
    LOGGER.status("Generate PSP target cell groups:\n {}".format(args))

    LOGGER.status("LOCATION {}".format(Path(__file__).parent))

    protocol = args.simulation_protocol or "single-spike"
    ...
    LOGGER.status("Generate pathways",
                  ...
                  "simulation protocol {}".format(protocol),
    ...
    for target in targets:
        ...
        psp.config.generate_psp_targets(path_pathways,
                                        cell_group={"$target": target},
