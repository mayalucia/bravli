...
def path_project(number):
    ...
    """
    return Path("/gpfs/bbp.cscs.ch/project/proj{}".format(number))

path_rat_sscx_dissemination = path_project(83)

path_rat_sscx_dissemination_atlases = path_project(83).joinpath("data",
                                                                "atlas",
                                                                "S1/MEAN")
path_atlas = {
    "O1MouseSSCx": path_project(66).joinpath("entities/dev/atlas/O1-152"),
    "O1RatSSCxDiss": path_project(83).joinpath("data/atlas/O1/MEAN/mean"),
    "S1RatSSCx": path_project(64).joinpath("entities/dev/atlas",
                                           "fixed_77831ACA-6198-4AA0-82EF-D0475A4E0647_01-06-2018"),
    "O1MouseHip": path_project(42).joinpath("entities/dev/atlas/",
                                            "20190625-CA3-O1-atlas/"),
    "S1MouseNeoCx": path_project(68).joinpath("entities",
                                              "dev/atlas/ccf_2017-50um/20181114"),
    "S1RatSSCxDiss_BioM":  path_rat_sscx_dissemination_atlases.joinpath("P14-MEAN")
}
...
class CircuitAtlasTest(WithFields):
    ...
    def circuit_atlas(self):
        ...
        """
        return BlueBrainCircuitAtlas(path=str(self.path_atlas))
    ...
    def atlas(self):
        ...
        """
        return Atlas.open(str(self.path_atlas))
    ...
    def layer_mask(self):
        ...
        circuit_atlas_mask = self.circuit_atlas.get_mask(layer=layer)
            
        assert len(circuit_atlas_mask.shape) == 3
        assert np.nansum(circuit_atlas_mask) > 0
        atlas_mask = self.atlas\
    ...
    def multiple_layer_mask(self):
        ...
        circuit_atlas_mask = self.circuit_atlas\
                                 .get_mask(layer=[l for l, _ in layers_to_test])
        ...
        expect_equal(circuit_atlas_mask, atlas_mask,
                     ...
                     != atlas mask with {} Trues
                     """.format(np.nansum(circuit_atlas_mask),
    ...
    def region_layer_mask(self):
        ...
        atlas_region_mask = self.atlas\
                                .get_region_mask(atlas_region, attr="acronym")\
                                .raw
        atlas_layer_mask = self.atlas\
                               .get_region_mask(atlas_layer, attr="acronym")\
                               .raw
        atlas_mask = np.logical_and(atlas_region_mask,
                                    atlas_layer_mask)
        expect_equal(circuit_atlas_mask, atlas_mask)
    ...
    def multiple_region_multiple_layer_mask(self):
        ...
        circuit_atlas_mask = self.circuit_atlas\
                                 .get_mask(region=[r for r, _ in regions_to_test],
                                           layer=[l for l, _ in layers_to_test])
    ...
    def depth_mask(self):
        ...
        expect_equal(circuit_atlas_mask,
                     atlas_mask,
                     """
    ...
    def random_position_inside_volume(self,  number=1000):
        ...
        data volume.
        """
        regions_to_test = [r for r, _ in get_list(self.regions_to_test)]
        layers_to_test = [l for l, _ in get_list(self.layers_to_test)]
        ...
        for region in regions_to_test:
            for layer in layers_to_test:
                logger.info(
                    ...
                    """.format(region, layer))
                region_layer_mask = self.circuit_atlas\
                                        .get_mask(region=region,
                                                  layer=layer)
                random_positions = self.circuit_atlas\
                                       .random_positions(region=region,
                                                         layer=layer)
                layer_region_ids = self.circuit_atlas\
                                       .region_layer\
