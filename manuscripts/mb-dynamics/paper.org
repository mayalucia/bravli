#+TITLE: Network Dynamics of the /Drosophila/ Mushroom Body: Regime Classification, Neuromodulation, Stochasticity, and Model Invariance
...
#+LATEX_HEADER: \hypersetup{colorlinks=true,linkcolor=blue!60!black,citecolor=green!50!black,urlcolor=blue!70!black}
#+LATEX_HEADER: \bibliographystyle{plainnat}
#+LATEX_HEADER: \linespread{1.3}
#+LATEX_HEADER: \usepackage{parskip}
#+OPTIONS: toc:nil num:t author:t date:t

#+BEGIN_abstract
The FlyWire whole-brain connectome of /Drosophila melanogaster/ provides,
...
The completion of the /Drosophila/ whole-brain connectome by the FlyWire
...
is the primary locus of associative olfactory learning in /Drosophila/
...
goal is not biophysical realism but /computational insight/: what can
...
- *Regular spiking*: $a = 0$, $b = 0.5$ mV, $\tau_w = 100$ ms
- *Adapting*: $a = 0.1$ nS, $b = 2.0$ mV, $\tau_w = 300$ ms
- *Bursting*: $a = 0$, $b = 5.0$ mV, $\tau_w = 50$ ms
- *Fast spiking*: $a = 0$, $b = 0$, $\tau_w = 100$ ms (equivalent to exponential LIF)
...
1. *Release failure*: Each spike arriving at a synapse is transmitted
   ...
   ($p_{\text{rel}} \approx 0.1$--$0.5$), most spikes fail to elicit
   postsynaptic responses \citep{allen1994}.

2. *Intrinsic noise*: Gaussian current noise $\xi_i(t)$ is added to
...
#+ATTR_LATEX: :booktabs t :align llc
| State | Compartment modulation | Behavioural prediction |
|-------+----------------------------------+------------------------|
| Naive | All $m_c = 1.0$ | Neutral |
| Appetitive | Appetitive $m_c = 1.3$--$1.5$; aversive $m_c = 0.6$ | Approach |
...
2. *Temporal correlation*: Correlation of population rate time series
   (5 ms bins).
3. *Mean relative difference*: $\langle 2|r_{\text{LIF}} - r_{\text{AdEx}}| / (r_{\text{LIF}} + r_{\text{AdEx}}) \rangle$ averaged over active neurons.

Interpretation thresholds: rate correlation $> 0.9$ indicates topology
dominates; $< 0.5$ indicates the neuron model is essential.

* Results
** The FlyWire Mushroom Body Operates in the Balanced State
...
** Neuromodulation Reconfigures Behavioural Output Without Rewiring
...
We test whether this principle extends to the /Drosophila/ MB.
...
** Stochastic Synaptic Transmission Serves Computation
...
*** Graceful Degradation of Odor Coding
...
*** Stochastic Resonance Enhances Signal Detection
...
** Topology Dominates: LIF and AdEx Agree When Adaptation Is Weak

\citet{zhang2024} demonstrated that connectome-constrained models of
the /Drosophila/ visual system predict neural responses accurately
...
*** Rate Correlation Across Neuron Models
...
*** Adaptation Strength Determines Divergence
...
** Synthesis: Four Views of One Circuit

The four investigations converge on a unified picture of the
/Drosophila/ MB as a circuit optimised for flexible, noise-tolerant
odor discrimination:
...
The FlyWire connectome transforms /Drosophila/ neuroscience from
